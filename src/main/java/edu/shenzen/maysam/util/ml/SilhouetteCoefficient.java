...
public class SilhouetteCoefficient {
    ...
    public double silhouetteCoefficient(Map<Integer, Centroid> clusterPointsByID) {
    ...
    private double minInterClusterDissimilarityForPoint(
        ...
        return clusteredPointsMap.entrySet().stream().mapToDouble(entry -> {
            ...
            return clusterDissimilarityForPoint(point, new ArrayList<>(entry.getValue().getTrainingSamples().values()), false);
    ...
    private double clusterDissimilarityForPoint(Point point,
        ...
        for (Point clusterPoint : clusterPoints) {
            totalDissimilarity += clusterPoint.findDistance(point, DistanceSet.EUCLIDEAN);
        }
